...
static inline int clear_user_sigset_extension(sigset_t __user *to){
	...
	int rc = clear_user(expansion, sizeof(_la_ow_sigset_t) - sizeof(sigset_t));
	if (rc < 0 ) {
		return -EFAULT;
	}
	return 0;
		...
		sigset_t __user *, oset, size_t, sigsetsize)
{
	if (sigsetsize == sizeof(sigset_t)) {
		...
		int rc = p_sys_rt_sigprocmask(how, nset, oset, sizeof(sigset_t));
		if (rc < 0) {
			return rc;
		}
		if (oset) {
...
__SYSCALL_DEFINEx(2, _rt_sigpending, sigset_t __user *, uset, size_t, sigsetsize)
{
	if (sigsetsize == sizeof(sigset_t)) {
		...
		size_t, sigsetsize)
{
	if (sigsetsize == sizeof(sigset_t)) {
		...
		int rc = p_sys_rt_sigtimedwait(uthese, uinfo, uts, sizeof(sigset_t));
		if (rc < 0) {
			return rc;
		}
		return rc;
...
__SYSCALL_DEFINEx(4, _rt_sigaction, int, sig,
...
__SYSCALL_DEFINEx(2, _rt_sigsuspend, sigset_t __user *, unewset, size_t, sigsetsize)
{
	if (sigsetsize == sizeof(sigset_t)) {
...
__SYSCALL_DEFINEx(6, _pselect6, int, n, fd_set __user *, inp, fd_set __user *, outp,
...
__SYSCALL_DEFINEx(6, _epoll_pwait, int, epfd, struct epoll_event __user *, events,
		...
		size_t, sizemask, int, flags)
{
	if (sizemask == sizeof(sigset_t)) {
                ...
                int rc = p_sys_signalfd4(ufd, user_mask, sizeof(sigset_t), flags);
		if (rc < 0) {
                        return rc;
                }
                return rc;
